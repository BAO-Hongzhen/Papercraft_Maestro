...
## 主要功能
  ...
  * **智能提示词系统**: **无需学习复杂的 Prompt！** 我们已内置经过精心调试的风格化提示词模板。用户只需输入想要生成的主体（例如："a rabbit" 或 "a dragon"），系统会自动将其填充到预设风格词之间：
    * **前缀**: "A vibrant red Chinese paper"
    * **用户输入**: [主体内容]
    * **后缀**: "complex Chinese patterns, stand proudly among the swirling clouds and stylized clouds. The background is pure white, emphasizing a bold traditional design" 通过这种自动拼接，系统确保了每次生成的图像都具备统一且纯正的中国传统剪纸风格。
...
| 模型类型 | 文件名 | 下载链接 | 存放路径 (ComfyUI根目录起) |
| :--- | :--- | :--- | :--- |
